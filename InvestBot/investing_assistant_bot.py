...
async def start(message: Message):
    ...
    "/diversification - узнать диверсификацию портфеля.\n"
    "/risk - узнать насколько рискованный ваш портфель.\n"
...
@dp.message(Command('premium_instruction'))
...
@dp.message(Command('subscribe_description'))
...
async def instruction_command(message: Message):
    await message.answer("Форматы ввода:\n"
        ...
        "Для удобства вводите названия пкркл знвчениями.")
...
@dp.message(Command("portfolio_type"))
...
@dp.message(Command("diversification"))
...
async def sharpe_cmd(message: Message):
    ...
    if std == 0:
        return await message.answer("Коэффициент Шарпа = ∞ (риск равен нулю).")
